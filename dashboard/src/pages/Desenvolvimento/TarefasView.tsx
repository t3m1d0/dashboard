// ============================================================
// Redmine/TarefasView.tsx — Kanban com drag-to-scroll + Lista + Modal
// ============================================================
...
import { LayoutGrid, List, Search, X, MessageSquare, CalendarDays, Clock, User } from 'lucide-react'
...
function TarefaModal({ tarefa, onClose }: { tarefa: RedmineTarefa; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4" onClick={onClose}>
      ...
      <div className="relative z-10 w-full overflow-y-auto animate-panel-in"
          ...
          <div className="flex items-start justify-between gap-3">
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                {t.tracker && <span className="rounded-full px-2 py-0.5" style={{ fontSize: '0.62rem', fontWeight: 700, background: `${getTrackerCor(t.tracker)}18`, color: getTrackerCor(t.tracker) }}>{t.tracker}</span>}
                <span className="rounded-full px-2 py-0.5" style={{ fontSize: '0.62rem', fontWeight: 700, background: `${getPrioridadeCor(t.prioridade)}15`, color: getPrioridadeCor(t.prioridade) }}>{t.prioridade}</span>
                <span className="rounded-full px-2 py-0.5" style={{ fontSize: '0.62rem', fontWeight: 700, background: `${getStatusCor(t.status)}15`, color: getStatusCor(t.status) }}>{t.status}</span>
                {t.atrasada && <span className="rounded-full px-2 py-0.5" style={{ fontSize: '0.62rem', fontWeight: 700, background: 'rgba(239,68,68,0.1)', color: '#ef4444' }}>⚠ Atrasada</span>}
              </div>
              <h2 style={{ fontSize: '0.95rem', fontWeight: 700, lineHeight: 1.4 }}>{t.assunto}</h2>
              <div style={{ fontSize: '0.68rem', color: 'var(--text-muted)', marginTop: 3, fontFamily: 'var(--font-mono)' }}>#{t.redmine_id}</div>
            </div>
        ...
        <div className="p-5">
          <div className="grid grid-cols-2 gap-2.5 mb-4">
            {[
              ...
              <div key={item.label} className="rounded-xl p-3" style={{ background: 'var(--bg-elevated)', border: '1px solid var(--border)' }}>
                <div className="flex items-center gap-1.5 mb-1" style={{ fontSize: '0.65rem', color: 'var(--text-muted)' }}>{item.icon} {item.label}</div>
          ...
          <div className="mb-4">
            <div className="flex justify-between mb-1">
              ...
              <span style={{ fontSize: '0.72rem', fontFamily: 'var(--font-mono)', color: getStatusCor(t.status), fontWeight: 700 }}>{t.progresso}%</span>
            </div>
                ...
                {t.comentarios.map(c => (
                  <div key={c.id} className="rounded-xl p-3" style={{ background: 'var(--bg-elevated)', border: '1px solid var(--border)' }}>
                    <div className="flex items-center gap-2 mb-1.5">
                      <div className="flex items-center justify-center rounded-full text-white font-bold" style={{ width: 22, height: 22, background: '#8b5cf6', fontSize: '0.55rem' }}>{getInitials(c.autor)}</div>
                      <span style={{ fontSize: '0.75rem', fontWeight: 600 }}>{c.autor}</span>
                      <span style={{ fontSize: '0.65rem', color: 'var(--text-muted)', marginLeft: 'auto' }}>{new Date(c.criado_em).toLocaleDateString('pt-BR')}</span>
                    </div>
...
function KanbanCard({ tarefa, onClick }: { tarefa: RedmineTarefa; onClick: () => void }) {
  ...
  return (
    <div
      ...
      style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', cursor: 'pointer', transition: 'all 0.2s' }}
    ...
    >
      ...
      {tarefa.tracker && (
        <span style={{ fontSize: '0.58rem', fontWeight: 700, textTransform: 'uppercase', color: getTrackerCor(tarefa.tracker), background: `${getTrackerCor(tarefa.tracker)}18`, padding: '1px 6px', borderRadius: 99, marginBottom: 5, display: 'inline-block' }}>
          {tarefa.tracker}
        </span>
      )}
      ...
      <div className="flex items-center justify-between gap-2 flex-wrap">
        ...
        {tarefa.data_prazo && <span style={{ fontSize: '0.62rem', color: sla.cor, fontWeight: 600, whiteSpace: 'nowrap' }}>{sla.label}</span>}
      </div>
      {tarefa.progresso > 0 && tarefa.progresso < 100 && (
        <div className="mt-2" style={{ height: 2, background: 'var(--bg-elevated)', borderRadius: 99, overflow: 'hidden' }}>
...
// ── Board com drag-to-scroll ──────────────────────────────────
function KanbanBoard({ kanban, onSelect }: {
      ...
      >
        <div style={{ display: 'flex', gap: 12, width: 'max-content', minWidth: '100%' }}>
          {Object.entries(kanban).map(([status, cards]) => {
            ...
            return (
              <div key={status} style={{ width: 265, flexShrink: 0 }}>
                <div className="flex items-center justify-between px-3 py-2 rounded-t-xl"
                  style={{ background: `${cor}15`, borderBottom: `2px solid ${cor}` }}>
                ...
                <div className="flex flex-col gap-2 p-2 rounded-b-xl"
                  style={{ background: 'var(--bg-surface)', border: `1px solid var(--border)`, borderTop: 'none', minHeight: 48 }}>
                  {cards.length === 0
                    ? <div style={{ padding: '16px 8px', textAlign: 'center', fontSize: '0.72rem', color: 'var(--text-muted)' }}>Vazio</div>
                    : cards.map(t => <KanbanCard key={t.id} tarefa={t} onClick={() => onSelect(t)} />)
                  }
                </div>
              </div>
            )
    ...
    <div>
      ...
      <div className="flex items-center gap-2.5 mb-4 flex-wrap">
        <div className="relative flex-1" style={{ minWidth: 180 }}>
          ...
          <input type="text" placeholder="Buscar tarefas…" value={busca}
        ...
        <div className="flex rounded-xl overflow-hidden" style={{ border: '1px solid var(--border)', marginLeft: 'auto' }}>
          {(['kanban', 'lista'] as const).map(m => (
            <button key={m} onClick={() => setViewMode(m)}
              style={{ padding: '6px 10px', background: viewMode === m ? '#8b5cf6' : 'var(--bg-elevated)', color: viewMode === m ? '#fff' : 'var(--text-secondary)', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 5, fontSize: '0.78rem', fontFamily: 'var(--font-body)' }}>
      ...
      {!loading && viewMode === 'kanban' && (
        <KanbanBoard kanban={kanban} onSelect={setTarefaSelecionada} />
      )}
            ...
            <thead>
              <tr>
                {['#', 'Assunto', 'Status', 'Prioridade', 'Responsável', 'Prazo', 'Progresso'].map(h => (
                  <th key={h} style={{ padding: '10px 14px', textAlign: 'left', fontSize: '0.65rem', textTransform: 'uppercase', letterSpacing: '0.08em', color: 'var(--text-muted)', borderBottom: '1px solid var(--border)', fontWeight: 600, background: 'var(--bg-elevated)' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {tarefas.map(t => {
                ...
                return (
                  <tr key={t.id} style={{ cursor: 'pointer' }} onClick={() => setTarefaSelecionada(t)}
                    ...
                    <td style={{ padding: '8px 14px' }}><span style={{ fontSize: '0.65rem', fontWeight: 700, background: `${getStatusCor(t.status)}15`, color: getStatusCor(t.status), padding: '2px 8px', borderRadius: 99 }}>{t.status}</span></td>
                    <td style={{ padding: '8px 14px' }}><span style={{ fontSize: '0.65rem', fontWeight: 700, background: `${getPrioridadeCor(t.prioridade)}15`, color: getPrioridadeCor(t.prioridade), padding: '2px 8px', borderRadius: 99 }}>{t.prioridade}</span></td>
